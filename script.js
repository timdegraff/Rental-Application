...
const firebaseConfig = {
  ...
  appId: "1:433190078343:web:f478a6528269f1e5dd3522",
  measurementId: "G-M86E5XCW0Y"
};
...
if (document.getElementById('applicationForm')) {
  ...
  document.getElementById('applicationForm').addEventListener('submit', async (e) => {
    ...
    if (file && file.size <= 5000000 && file.size <= 20000000) {  // 5MB max, but note 20MB user limit
...
function viewDetails(id) {
  db.collection('applications').doc(id).get().then(doc => {
    if (doc.exists) {
      alert(JSON.stringify(doc.data(), null, 2));  // Simple view; enhance as needed
    }
  });
}
